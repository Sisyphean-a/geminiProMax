...
function handleChatMutations(mutations) {
    ...
    const items = uniqueElements.map((el, index) => {
        ...
        if (!id) {
            ...
            if (!el.id)
                el.id = id; // Ensure it has an ID for scrolling
        }
            ...
            mutation.addedNodes.forEach((node) => {
                if (node instanceof HTMLElement) {
                    ...
                    if (node.tagName === 'MODEL-THOUGHTS' || node.querySelector('model-thoughts')) {
                        const targets = node.tagName === 'MODEL-THOUGHTS' ? [node] : node.querySelectorAll('model-thoughts');
...
function injectTranslator(thoughtsRoot) {
    ...
    if (!contentDiv)
        contentDiv = thoughtsRoot.querySelector('.mat-expansion-panel-body');
    if (!contentDiv && headerBtn.nextElementSibling)
        contentDiv = headerBtn.nextElementSibling;
    if (contentDiv) {
...
function observeForContent(thoughtsRoot) {
    if (activeThoughtObservers.has(thoughtsRoot))
    ...
    const observer = new MutationObserver((mutations) => {
        ...
        if (!contentDiv)
            contentDiv = thoughtsRoot.querySelector('.mat-expansion-panel-body');
        let headerBtn = thoughtsRoot.querySelector('button[data-test-id="thoughts-header-button"]') ||
            thoughtsRoot.querySelector('button[aria-label*="thoughts"]');
